...
W_SPATIAL         = 0.30        # composite weight
W_POSE            = 0.30
W_DEPTH           = 0.25
W_RADIAL          = 0.15
GRADE_A_THRESH    = 0.85        # composite score for auto-stop

# ── Depth / Scale diversity configuration ──
DEPTH_NEAR_SCALE  = 0.30        # board linear scale fraction >= 30% (Near)
DEPTH_FAR_SCALE   = 0.16        # board linear scale fraction < 16% (Far)
DEPTH_MIN_SAMPLES = 6           # min frames per depth bucket for full depth score
...
def compute_board_scale(
    corners_2d: np.ndarray,
    img_w: int,
    img_h: int,
) -> float:
    """
    Computes the linear scale fraction of the checkerboard relative to the frame.
    Uses the polygon area of the convex hull of the corners.

    Args:
        corners_2d: (N, 2) or (N, 1, 2) float32 coordinates.
        img_w, img_h: frame dimensions in pixels.

    Returns:
        scale in [0.0, 1.0], representing sqrt(board_area / (img_w * img_h)).
    """
    pts = corners_2d.reshape(-1, 2)
    if len(pts) < 4 or img_w <= 0 or img_h <= 0:
        return 0.0
    ...
    if frame_area <= 0:
        return 0.0
    return float(math.sqrt(max(0.0, area) / frame_area))


def coverage_grade(score: float) -> str:
    """Map composite score to letter grade."""
    if score >= GRADE_A_THRESH: return "A"
...
class CalibState:
    ...
    def _reset_locked(self) -> None:
        ...
        # Depth / scale metrics
        self.depth_counts = {"near": 0, "mid": 0, "far": 0}
        self.depth_min_z  = 0.0
        self.depth_max_z  = 0.0
        self.scale_min    = 1.0
        self.scale_max    = 0.0
    ...
    def update_depth_incremental(
        ...
        """Track scale / metric depth and compute multi-bucket depth diversity score."""
        scale = compute_board_scale(corners, img_w, img_h)
        if scale > 0:
            self.scale_min = min(self.scale_min, scale)
            self.scale_max = max(self.scale_max, scale)

            if scale >= DEPTH_NEAR_SCALE:
                self.depth_counts["near"] += 1
            elif scale < DEPTH_FAR_SCALE:
                self.depth_counts["far"] += 1
            else:
                self.depth_counts["mid"] += 1

        if tvec is not None:
            tz = float(np.asarray(tvec).flatten()[2])
            if tz > 0:
                if self.depth_min_z == 0.0 or tz < self.depth_min_z:
                    self.depth_min_z = tz
                if tz > self.depth_max_z:
                    self.depth_max_z = tz

        near_frac = min(1.0, self.depth_counts["near"] / DEPTH_MIN_SAMPLES)
        mid_frac  = min(1.0, self.depth_counts["mid"] / DEPTH_MIN_SAMPLES)
        far_frac  = min(1.0, self.depth_counts["far"] / DEPTH_MIN_SAMPLES)

        self.depth = float((near_frac + mid_frac + far_frac) / 3.0)
    ...
    def snapshot(self) -> Dict:
        ...
        with self._lock:
            return dict(
                spatial=self.spatial, pose=self.pose, depth=self.depth, radial=self.radial,
                ...
                depth_counts=self.depth_counts.copy(),
                depth_min_z=self.depth_min_z,
                depth_max_z=self.depth_max_z,
    ...
    def _update_hint_locked(self) -> None:
        ...
        if self.composite >= GRADE_A_THRESH:
        ...
        elif self.depth < 0.60:
            if self.depth_counts["far"] < DEPTH_MIN_SAMPLES:
                self.hint = "Step back — need more FAR distance frames"
            elif self.depth_counts["near"] < DEPTH_MIN_SAMPLES:
                self.hint = "Bring board closer — need more NEAR distance frames"
            else:
                self.hint = "Hold at medium distance — need MID distance frames"
            ...
            self.hint = (f"Looking good — {int(self.composite * 100)}%"
                         " composite, keep varying pose & depth")
...
def draw_hud(
    ...
    bar_rows = [
        ("Spatial", snap["spatial"], C_GREEN,  f"×{W_SPATIAL:.2f}"),
        ("Pose",    snap["pose"],    C_BLUE,   f"×{W_POSE:.2f}"),
        ("Depth",   snap["depth"],   C_CYAN,   f"×{W_DEPTH:.2f}"),
        ("Radial",  snap["radial"],  C_ORANGE, f"×{W_RADIAL:.2f}"),
    ]
    by = gy2 + 38
    ...
    # Depth tiers indicator
    d_cnt = snap.get("depth_counts", {})
    n_c = d_cnt.get("near", 0)
    m_c = d_cnt.get("mid", 0)
    f_c = d_cnt.get("far", 0)
    n_sym = "✓" if n_c >= DEPTH_MIN_SAMPLES else str(n_c)
    m_sym = "✓" if m_c >= DEPTH_MIN_SAMPLES else str(m_c)
    f_sym = "✓" if f_c >= DEPTH_MIN_SAMPLES else str(f_c)
    cv.putText(frame, f"Z tiers: N[{n_sym}] M[{m_sym}] F[{f_sym}]",
               (bar_x_label, by + 20), FONT, 0.38, (140, 140, 140), THICK_1, cv.LINE_AA)
...
def collector_thread_fn(
    ...
    while not stop_event.is_set() or not result_q.empty():
        ...
        with state._lock:
            ...
            state.update_depth_incremental(
                corners2,
                img_w,
                img_h,
                tvec=tvec
...
def main(override_args: Optional[List[str]] = None) -> None:
    ...
    def _submit_and_forward(item):
        ...
        corners2, rvec, tvec = detect_frame(item)
        result_q.put((corners2, rvec, tvec))
    ...
    while True:
        ...
        # Auto-stop when grade A / 0.85 threshold is reached
        if (last_snap["grade"] == "A" or last_snap["composite"] >= GRADE_A_THRESH) and recording:
            print(f"\n✅  Grade A reached ({last_snap['composite']*100:.1f}%) — stopping recording and starting calibration automatically.")
            recording = False
            if video_writer is not None:
                video_writer.release()
                print(f"💾  [recording] Video saved: {video_path} ({video_frames_written} frames)")
